...
export default function DigitizerTaskBoard() {
  ...
  const fetchDigitizingOrders = useCallback(async () => {
    try {
      setLoading(true);
      ...
      // Filter for Digitizing specific statuses when viewing 'all' or 'available'
      if (viewTab === "all" || viewTab === "available") {
        filtered = filtered.filter((o) =>
          ["PENDING_DIGITIZING"].includes(o.status),
        );
      }
  ...
  const claimTask = async (orderId) => {
    try {
      ...
      await fetchDigitizingOrders(); // Refresh
    } catch (err) {
  ...
  const finishTask = async (orderId) => {
    if (
      !window.confirm(
        "คุณแน่ใจหรือไม่ว่าอัปโหลดไฟล์ตีลาย (.emb) ระบุสีด้าย ครบถ้วนแล้ว และต้องการส่งมอบงาน?",
      )
    ) {
      return;
    }
    try {
      ...
      await api.patch(
        ...
        { status: "PENDING_ARTWORK", subStatus: "ตีลายเสร็จสิ้น" },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      await fetchDigitizingOrders();
    } catch (err) {
      alert(err.response?.data?.message || "ไม่สามารถอัปเดตสถานะงานได้");
    } finally {
      setClaiming(null);
    }
  };
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative group">
                <HiOutlineMagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-600 transition-colors" />
                <input
                  type="text"
                  placeholder="ค้นหา Job ID หรือชื่อลูกค้า..."
                  className="pl-10 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400 transition-all w-64 text-xs font-bold"
                  value={search}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <div
                              className={`w-1.5 h-1.5 rounded-full ${order.digitizer ? "bg-emerald-500" : "bg-slate-300 animate-pulse"}`}
                            ></div>
                            <span className="text-[11px] font-bold text-slate-600">
                              {order.digitizer
                                ? order.digitizer.name
                                : "รอรับงาน..."}
                            </span>
                          </div>
                        </td>
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex flex-col gap-1.5 w-full">
                        <div className="flex justify-between items-start">
                          ...
                          <span
                            className={`px-2 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg border flex-shrink-0 text-center ${order.status === "PENDING_DIGITIZING" ? "bg-amber-100 text-amber-600 border-transparent" : "bg-slate-100 text-slate-400 border-slate-200"}`}
                          >
                            {getStatusLabel(order.status)}
                          </span>
                        </div>
