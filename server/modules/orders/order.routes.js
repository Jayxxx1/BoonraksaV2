...
router.route('/channels')
  .get(orderController.getSalesChannels);

router.route('/search/:jobId')
  .get(orderController.searchOrderByJobId);

// Status-based Aliases (Compatibility with OrderStatusBar.jsx)
router.route('/:id/status').patch(validate(updateStatusSchema), orderController.updateStatus);
router.route('/:id/PENDING_STOCK_CHECK').patch(orderController.printJobSheetSignal);
...
router.route('/:id/claim').post(orderController.claimTask);
