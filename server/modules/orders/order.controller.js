...
export const getOrder = asyncHandler(async (req, res) => {
  ...
  if (!order) return sendError(res, OrderErrorCodes.ORDER_NOT_FOUND, 'Order not found', 404);
...
export const searchOrderByJobId = asyncHandler(async (req, res) => {
  ...
  if (!order) return sendError(res, OrderErrorCodes.ORDER_NOT_FOUND, 'ไม่พบเลขออเดอร์ (JOB ID) นี้ในระบบ', 404);
  sendSuccess(res, { order });
});
...
export const updateStatus = asyncHandler(async (req, res) => {
  const { id } = req.params;
  try {
    const order = await orderService.updateStatus(id, req.body.status, req.user, req.body);
    sendSuccess(res, { order }, `Order status updated to ${req.body.status}`);
  } catch (error) {
    if (error.message === 'PAYMENT_INCOMPLETE') return sendError(res, 'PAYMENT_INCOMPLETE', 'ยังชำระเงินไม่ครบ ไม่สามารถปิดงานได้');
    if (error.message === 'TRACKING_REQUIRED') return sendError(res, 'TRACKING_REQUIRED', 'กรุณาระบุเลขพัสดุ');
    throw error;
  }
});
...
export const claimTask = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (error.message === 'UNAUTHORIZED_ACTION') return sendError(res, OrderErrorCodes.UNAUTHORIZED_ACTION, 'สิทธิ์ของคุณไม่สามารถรับงานนี้ได้', 403);
    throw error;
  }
});
